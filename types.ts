...
export interface User {
  ...
  // BOARD EXAM SPECIAL
  completedBoardNotes?: string[]; // IDs of completed BoardNotes
}
...
export interface BoardNote {
  ...
  content: string; // The raw text with tags [H1], [INT] etc.
  price: number; // 0 for Free/Subscription
...
export interface SystemSettings {
  ...
  chatMode?: 'PRIVATE_ONLY' | 'UNIVERSAL_ONLY' | 'BOTH'; // NEW: Chat Visibility Control
  ...
  noticeText?: string;    // NEW: Admin Notification
  ...
  // GAME CONFIG
  wheelRewards: SpinReward[]; // Updated to support Objects

  chatCost: number;       // Default 1. If 0, it's free.
  chatCooldownHours?: number; // NEW: Cooldown
  ...
  // MCQ PRICING
  mcqTestCost?: number;       // Default 2
  mcqAnalysisCost?: number;   // Default 5
  mcqHistoryCost?: number;    // Default 1 (New Request)
  enableMcqUnlockRestriction?: boolean; // NEW: 100 MCQ Rule Toggle
  profileEditCost?: number;   // NEW: Cost to edit profile (Free for Premium)
  ...
  // FEATURED SHORTCUTS
  featuredSectionTitle?: string; // "What do you want to study?"
...
export type StudentTab = 'HOME' | 'SESSION' | 'COURSES' | 'ROUTINE' | 'CHAT' | 'HISTORY' | 'REDEEM' | 'PREMIUM' | 'GAME' | 'WEEKLY_TEST' | 'PROFILE' | 'LEADERBOARD' | 'STORE' | 'VIDEO' | 'PDF' | 'MCQ' | 'ANALYTICS' | 'BOARD_EXAM' | 'MARGDARSHAN' | 'PRIZES' | 'REWARDS';
